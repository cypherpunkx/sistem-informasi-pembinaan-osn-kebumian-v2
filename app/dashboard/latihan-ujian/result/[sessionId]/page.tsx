    ...
    return (
        <div className="max-w-4xl mx-auto space-y-8 pb-10">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-neutral-warm/20 p-8 text-center relative overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-accent-earthy to-green-600"></div>
                ...
                <div className="flex justify-center items-center gap-8 mb-8">
                    <div className="text-center">
                        <div className="text-4xl font-bold text-accent-earthy mb-1">{session.score}</div>
                        <div className="text-sm text-text-dark/60 uppercase tracking-wide font-medium">Score</div>
                    </div>
                    ...
                    <div className="text-center">
                        <div className="text-2xl font-bold text-text-dark mb-1 flex justify-center items-center gap-2">
                            {session.correctAnswers} <span className="text-base font-normal text-text-dark/40">/ {session.totalQuestions}</span>
                        </div>
                        <div className="text-sm text-text-dark/60 uppercase tracking-wide font-medium">Correct</div>
                    </div>
                    <div className="h-12 w-px bg-neutral-warm/30"></div>
                    <div className="text-center">
                        <div className="text-2xl font-bold text-text-dark mb-1 flex justify-center items-center gap-2">
                        ...
                        <div className="text-sm text-text-dark/60 uppercase tracking-wide font-medium">Duration</div>
                    </div>

                </div>

                <Link href="/dashboard/latihan-ujian" className="inline-flex items-center gap-2 text-text-dark/60 hover:text-accent-earthy transition-colors font-medium">
            ...
            <div className="space-y-6">
                <h2 className="text-xl font-bold text-text-dark border-l-4 border-accent-earthy pl-4">Detailed Review</h2>

                {results?.map((item: any, index: number) => {
                    const isCorrect = item.isCorrect;
                    const userAnswerText = item.userAnswer ? item.options.find((o: any) => o.id.toString() === item.userAnswer.toString())?.content : "No Answer";

                    return (
                        <div key={item.id} className={`bg-white rounded-xl shadow-sm border p-6 transition-all 
                                    ...
                                    <div>
                                        <div className="flex justify-between items-start mb-4">
                                            ...
                                            <div className="flex flex-col items-end">
                                                <span className={`text-sm font-bold px-2 py-1 rounded border
                                                    ${item.earnedScore === item.weight ? 'bg-green-100 text-green-700 border-green-200' :
                                                        item.earnedScore > 0 ? 'bg-yellow-100 text-yellow-700 border-yellow-200' :
                                            ...
                                            <div className="space-y-2">
                                                {item.options && item.options.length > 0 ? (
                                                    item.options.map((option: any, optIdx: number) => {
                                                        const isSelected = item.userAnswer === option.id.toString();
                                                        const isActuallyCorrect = option.isCorrect === true;

                                                        let optionClass = "border-neutral-warm/20 bg-white";
                                                        let icon = null;
                                                        ...
                                                        return (
                                                            <div key={option.id} className={`p-3 rounded-lg border flex items-center justify-between ${optionClass}`}>
                                                                <div className="flex items-center gap-3">
                                                                    <div className={`w-6 h-6 rounded-full border flex items-center justify-center text-xs font-bold
                                                                        ${isSelected || isActuallyCorrect ? 'border-transparent bg-black/5' : 'border-neutral-warm/40'}`}>
                                                                        {String.fromCharCode(65 + optIdx)}
                                                                    </div>
                                                                    <span className="text-text-dark">{option.content}</span>
                                                                </div>
                                                                {icon}
                                                            </div>
                                                        );
                                        ...
                                        {item.type === "SHORT_ANSWER" && (
                                            <div className="space-y-3 mt-4">
                                                <div>
                                                    <p className="text-xs font-bold text-text-dark/60 uppercase mb-1">Your Answer</p>
                                                ...
                                                {!isCorrect && (
                                                    <div>
                                                        <p className="text-xs font-bold text-text-dark/60 uppercase mb-1">Accepted Answers</p>
                                                        <div className="p-3 rounded-lg border border-neutral-warm/20 bg-white text-sm text-text-dark">
                                                            {item.answerKeys?.map((k: any) => `${k.key} (${k.type})`).join(", ") || "No Key"}
                                                        </div>
                                                    </div>
                                                )}
                                            </div>
                                                ...
                                                {item.feedback && (
                                                    <div className="bg-accent-earthy/5 p-4 rounded-lg border border-accent-earthy/10">
                                                        <p className="text-xs font-bold text-accent-earthy uppercase mb-2 flex items-center gap-2">
                                                            <CheckCircle className="w-3 h-3" /> Auto-Grading Feedback
                                                        </p>
