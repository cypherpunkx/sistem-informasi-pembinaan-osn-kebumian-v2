...
import { getExamSession } from "@/app/actions/exams";
...
export default async function ExamResultPage({
    ...
    return (
        <div className="flex flex-col items-center justify-center p-8 bg-white rounded-xl shadow-sm border border-neutral-warm/20 text-center max-w-2xl mx-auto mt-10">
            <div className={`w-24 h-24 rounded-full flex items-center justify-center mb-6 bg-current ${score >= 60 ? 'text-green-100' : 'text-red-100'}`}>
                {score >= 60 ? (
                    <CheckCircle className={`w-12 h-12 ${score >= 60 ? 'text-green-600' : 'text-red-600'}`} />
                ) : (
                    <XCircle className="w-12 h-12 text-red-600" />
                )}
            </div>

            <h1 className="text-3xl font-bold text-text-dark mb-2">Exam Completed!</h1>
            ...
            <div className="bg-neutral-light/30 rounded-xl p-8 w-full mb-8">
                <p className="text-text-dark/60 text-sm uppercase tracking-wider font-bold mb-2">Your Score</p>
                <div className={`text-6xl font-black ${gradeColor}`}>
                    {score}
                </div>
                <p className={`mt-2 font-medium ${gradeColor}`}>
                    {gradeMessage}
                </p>
            </div>
