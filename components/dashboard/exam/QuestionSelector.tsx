...
export default function QuestionSelector({ selectedIds, onSelectionChange }: QuestionSelectorProps) {
            ...
            <div className="max-h-60 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                {questions.map((q) => {
                    ...
                    return (
                        ...
                        >
                            <div className={`mt-0.5 ${isSelected ? "text-accent-earthy" : "text-neutral-400"}`}>
                                {isSelected ? <CheckCircle className="w-5 h-5" /> : <PlusCircle className="w-5 h-5" />}
                            </div>
                            <div className="flex-1">
                                <p className="text-sm text-text-dark line-clamp-2">{q.content}</p>
                                <div className="flex gap-2 mt-1">
                                    <span className="text-xs px-1.5 py-0.5 bg-neutral-warm/10 rounded text-text-dark/60">{q.topic}</span>
                                    <span className="text-xs px-1.5 py-0.5 bg-neutral-warm/10 rounded text-text-dark/60">{q.type}</span>
                                </div>
                            </div>
                        </div>
                    );
                ...
                {questions.length === 0 && !loading && (
                    <p className="text-center text-sm text-text-dark/50 py-4">No questions found.</p>
                )}
                {loading && (
                    <p className="text-center text-sm text-text-dark/50 py-4">Loading...</p>
                )}
            </div>
