...
export default function LaporanView({
  ...
  const localPeriod = draftPeriod ?? String(periodDays);
  const localCategory = draftCategory ?? category;
  const localType = draftType ?? type;
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-neutral-warm/20 flex items-center justify-between animate-in fade-in duration-300">
          <div>
            <p className="text-sm text-text-dark/60 font-medium">Total Sesi</p>
            <p className="text-3xl font-bold text-text-dark mt-2 tabular-nums">
              {summary.totalSessions.toLocaleString('id-ID')}
            </p>
            <p className="text-xs text-text-dark/50 mt-2">pengerjaan ujian</p>
          </div>
          <div className="p-3 bg-violet-100 rounded-lg shrink-0">
            <ClipboardList className="w-6 h-6 text-violet-600" aria-hidden />
          </div>
        </div>
        <div className="bg-white p-6 rounded-xl shadow-sm border border-neutral-warm/20 flex items-center justify-between animate-in fade-in duration-300">
          <div>
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        <div className="bg-white p-6 rounded-xl shadow-sm border border-neutral-warm/20">
          ...
          <p className="text-sm text-text-dark/60 mt-1 mb-4">
            Aktivitas ujian (jumlah sesi dikerjakan per hari) — fokus pembinaan
            engagement.
          </p>
          <div className="relative h-28 flex items-end gap-px">
            {trend.length === 0 ? (
              <p className="text-sm text-gray-500 self-center w-full text-center">
                Tidak ada data
              </p>
            ) : (
              <>
                {/* Grid lines */}
                <div
                  className="absolute inset-0 flex flex-col justify-between pointer-events-none pl-6"
                  aria-hidden
                >
                  {[0, 1, 2, 3, 4].map((i) => (
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-neutral-warm/20">
          ...
          <p className="text-sm text-text-dark/60 mt-1 mb-4">
            Jumlah sesi ujian (attempt) per rentang nilai dalam periode ini.
          </p>
        ...
        <div className="space-y-2">
          {examRows.length === 0 ? (
            ...
            examRows.slice(0, 10).map((r) => (
              <div key={r.examId} className="flex items-center gap-3 gap-x-4">
